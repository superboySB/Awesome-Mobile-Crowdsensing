...
for ((i = 0; i < train_num; i++)); do
  ...
  --gpu_id ${cards[card_id]} ${trains[i]} --use_2d_state --with_programming_optimization --tag large_shift"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
