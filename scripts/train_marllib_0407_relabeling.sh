...
trains=(
  ...
  "--dataset SanFrancisco --reward_mode original --rl_gamma 0 --use_relabeling --relabel_threshold 0.25"
  "--dataset SanFrancisco --reward_mode original --rl_gamma 0 --use_relabeling --relabel_threshold 0.5"
  "--dataset SanFrancisco --reward_mode original --rl_gamma 0 --use_relabeling --relabel_threshold 0.75"
)
...
for ((i = 0; i < train_num; i++)); do
  ...
  --display_tags reward_mode relabel_threshold"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
