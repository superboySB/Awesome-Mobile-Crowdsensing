...
for ((i = 0; i < train_num; i++)); do
  ...
  --gpu_id ${cards[card_id]} ${trains[i]} --use_2d_state --tag buffer_in_obs attention_only --look_ahead --buffer_in_obs\
   --separate_encoder --intrinsic_mode scaled_dis_aoi --reward_mode greedy --emergency_threshold 20"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
