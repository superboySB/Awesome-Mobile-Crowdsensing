#!/bin/bash
exp_name='75_pcgrad_and_penalty'
...
trains=(
  "--dataset SanFrancisco --tag restore_verify --prioritized_buffer --reward_mode greedy --rl_gamma 0"
  "--dataset SanFrancisco --tag restore_verify --prioritized_buffer --rl_use_cnn --reward_mode original --rl_gamma 0.5 --surveillance_penalty 0"
  "--dataset SanFrancisco --prioritized_buffer --rl_use_cnn --reward_mode original --rl_gamma 0.5 --surveillance_penalty 0 --use_pcgrad"
  "--dataset SanFrancisco --prioritized_buffer --rl_use_cnn --reward_mode original --rl_gamma 0.5 --surveillance_penalty 1e-6"
  "--dataset SanFrancisco --prioritized_buffer --rl_use_cnn --reward_mode original --rl_gamma 0.5 --surveillance_penalty 1e-5"
  "--dataset SanFrancisco --prioritized_buffer --rl_use_cnn --reward_mode original --rl_gamma 0.5 --surveillance_penalty 1e-4"
)
...
for ((i = 0; i < train_num; i++)); do
  ...
  --emergency_threshold 20 --speed_discount 0.5 --selector_type RL --use_random\
  --emergency_queue_length 5 --gen_interval 10 --intrinsic_mode scaled_dis_aoi --sibling_rivalry --alpha 0.3\
  --display_tags reward_mode rl_gamma surveillance_penalty"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
