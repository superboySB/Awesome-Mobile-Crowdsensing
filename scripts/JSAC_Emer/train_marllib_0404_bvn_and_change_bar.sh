...
trains=(
  ...
  "--dataset SanFrancisco --tag restore_verify --rl_use_cnn --reward_mode original --rl_gamma 0.5"
  "--dataset SanFrancisco --rl_use_cnn --reward_mode original --rl_gamma 0.5 --use_bvn"
  "--dataset SanFrancisco --rl_use_cnn --reward_mode original --rl_gamma 0.5 --surveillance_penalty 1e-5 --surveillance_threshold 30"
  "--dataset SanFrancisco --rl_use_cnn --reward_mode original --rl_gamma 0.5 --surveillance_penalty 1e-5 --surveillance_threshold 20"
  "--dataset SanFrancisco --rl_use_cnn --reward_mode original --rl_gamma 0.5 --surveillance_penalty 1e-5 --surveillance_threshold 10"
)
...
for ((i = 0; i < train_num; i++)); do
  ...
  --display_tags reward_mode rl_gamma surveillance_threshold"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
