...
trains=(
  "--dataset SanFrancisco --tag restore_verify --prioritized_buffer --reward_mode greedy"
  "--dataset SanFrancisco --tag restore_verify --prioritized_buffer --buffer_in_obs --reward_mode greedy"
  "--dataset SanFrancisco --prioritized_buffer --rl_use_cnn --reward_mode original"
  "--dataset SanFrancisco --prioritized_buffer --rl_use_cnn --buffer_in_obs --reward_mode original"
  "--dataset SanFrancisco --prioritized_buffer --rl_use_cnn --use_action_mask --reward_mode original"
  "--dataset SanFrancisco --prioritized_buffer --rl_use_cnn --buffer_in_obs --use_action_mask --reward_mode original"
)
...
for ((i = 0; i < train_num; i++)); do
  ...
  --emergency_threshold 20 --speed_discount 0.5 --selector_type RL --rl_gamma 0 --use_random\
  --emergency_queue_length 5 --gen_interval 10 --intrinsic_mode scaled_dis_aoi --sibling_rivalry --alpha 0.3\
  --display_tags dataset reward_mode rl_gamma"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
