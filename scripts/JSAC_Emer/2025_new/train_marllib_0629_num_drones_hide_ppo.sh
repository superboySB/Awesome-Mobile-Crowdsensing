#!/bin/bash
exp_name='50_num_drones_hide_ppo'
# not completely edited.
session_name=$exp_name
cards=(1 2 3 4 4 3 4 5 6 7 8 8 9 7)
card_num=${#cards[@]}
dry_run=false
...
trains=(
  "--dataset SanFrancisco --num_drones 20"
...
for ((i = 0; i < train_num; i++)); do
  ...
  --num_cars 0 --group 2025_resubmit --algo ippo --share_policy all --switch_step 60000000\
  ...
  --emergency_queue_length 1 --intrinsic_mode none"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
