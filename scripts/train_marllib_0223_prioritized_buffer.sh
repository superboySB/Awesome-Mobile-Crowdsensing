...
exp_name='75_prioritized'
session_name=$exp_name
cards=(4 4 1 2 3 5 6 7)
card_num=${#cards[@]}
dry_run=false
# Process command-line ssharguments
while [[ $# -gt 0 ]]; do
    case "$1" in
        --dry-run)
            dry_run=true
...
trains=(
  ...
  "--dataset SanFrancisco --selector_type RL greedy --emergency_queue_length 10 --look_ahead"
  "--dataset SanFrancisco --selector_type RL greedy --emergency_queue_length 8 --look_ahead"
  "--dataset SanFrancisco --selector_type RL greedy --emergency_queue_length 5 --look_ahead"
  "--dataset SanFrancisco --selector_type RL greedy --emergency_queue_length 3 --look_ahead"
)
...
for ((i = 0; i < train_num; i++)); do
  ...
  --gpu_id ${cards[card_id]} ${trains[i]} --use_2d_state --tag prioritized_buffer --prioritized_buffer"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
