...
trains=(
  ...
  "--dataset SanFrancisco --selector_type greedy --gen_interval 10"
  "--dataset SanFrancisco --selector_type random --gen_interval 10"
  "--dataset Chengdu --selector_type RL --gen_interval 10 --reward_mode mix"
  "--dataset Chengdu --selector_type RL --gen_interval 10 --reward_mode original"
  "--dataset Chengdu --selector_type RL --gen_interval 10 --reward_mode intrinsic"
  "--dataset Chengdu --selector_type RL --gen_interval 10 --reward_mode none"
  "--dataset Chengdu --selector_type greedy --gen_interval 10"
  "--dataset Chengdu --selector_type random --gen_interval 10"
)
...
for ((i = 0; i < train_num; i++)); do
  ...
  --gpu_id ${cards[card_id]} ${trains[i]} --use_2d_state --look_ahead --rl_gamma 0.5 --with_programming_optimization\
  --no_refresh --emergency_queue_length 1 --tag no_refresh"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
