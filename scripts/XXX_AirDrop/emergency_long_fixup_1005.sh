#!/bin/bash
exp_name='emergency-with-assign-long'
...
for ((i = 0; i < train_num; i++)); do
  ...
  command="CUDA_VISIBLE_DEVICES=${cards[card_id]} python verification/uav_parachute_ugv_test.py --track --name ${exp_name} ${trains[i]} --num-episodes 80000"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
