...
trains=(
  ...
  "--dataset SanFrancisco --reward_mode greedy --rl_gamma 0 --use_neural_ucb"
  "--dataset SanFrancisco --reward_mode original --rl_gamma 0.5"
  "--dataset SanFrancisco --reward_mode original --rl_gamma 0.5 --use_neural_ucb"
  "--dataset SanFrancisco --reward_mode original --rl_gamma 0"
  "--dataset SanFrancisco --reward_mode original --rl_gamma 0 --use_neural_ucb"
)
...
for ((i = 0; i < train_num; i++)); do
  ...
  --display_tags reward_mode rl_gamma"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
