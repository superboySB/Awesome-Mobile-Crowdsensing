...
for ((i = 0; i < train_num; i++)); do
  ...
  --gpu_id ${cards[card_id]} ${trains[i]} --use_2d_state --tag tune_alpha --look_ahead --with_programming_optimization\
  ...
  --emergency_queue_length 3 --display_tags dataset emergency_queue_length alpha"
  echo "$command"
  if [ "$dry_run" = "false" ] && [ "$choice" != "n" ]
  then
