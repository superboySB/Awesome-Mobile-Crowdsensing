...
class CrowdSim:
    ...
    def __init__(
        ...
        """
        Initialize the environment for traffic monitoring multi-task-oritned MCS.

        :param num_drones: Number of drones
        :param num_cars: Number of cars
        :param seed: Seed for the random number generator
        :param env_backend: Backend for the environment (CPU or CUDA)
        :param single_type_agent: Use only one type of agent (car or drone)
        :param dynamic_zero_shot: Generate emergencies PoIs (whose locations are randomle selected)
        :param use_2d_state: Use 2D state representation (and CNN encoder)
        :param fix_target: Fix the target locations of PoIs
        :param env_config: Environment configuration (use custom config class)
        :param centralized: Use centralized reward
        :param all_random: All PoIs are randomly generated
        :param cut_points: Reduce numbers of surveillance PoIs to a certain threshold if exceeded
        :param gen_interval: the interval (timeslots) for generating emergency PoIs
        :param points_per_gen: the emergency points generated at each round (related with gen_interval)
        :param no_refresh: use same set of emergency PoIs for all training.
        By default, emergency PoIs are refreshed every single episode.
        :param emergency_threshold: the AoI threshold for emergency.
        :param surveillance_threshold: the AoI threshold for surveillance.
        :param emergency_queue_length: the length of the emergency queue (or dynamically weighted buffer)
        :param force_allocate: agent will receive reward only when it handles a emergency PoI allocated to it.
        :param with_end_time: When enabled, emergency PoIs can only be handled when AoI is below threshold.
        :param buffer_in_obs: append buffer features into agent observation.
        :param intrinsic_mode: the mode for intrinsic reward, detailed mode can be seen in command line arguments.
        :param use_random: use random generated emergency PoIs instead of collected PoIs from dataset.
        :param speed_action: enable agent to controll its speed.
        :param blur_requirement: specify the blur requirement (speed and time for handling emergency PoI) of image in
        handling emegency PoIs.
        :param emergency_reward: specify the reward received when a UAV handles one emergency
        within threshold or failing to handle one emergency (negative)
        :param refill_emergency: treat surveillance PoIs with high AoI as emergency PoI.
        :param surveillance_penalty: provide neagtive reward when agent fail to handle surveillance PoIs
    ...
    def get_data_dictionary(self):
        ...
        data_dict.add_data_list([("agent_types", self.int_dtype(self.agent_types)),  # [n_agents, ]
                                 ...
                                 ("agent_y", self.float_dtype(np.full([self.num_agents, ], self.starting_location_y)),
                                  True),
                                 ("agent_y_range", self.float_dtype(self.max_distance_y)),
...
tion_indices_arr[kThisAgentActionIdxOffset]];
      } else { // Drone Movement
