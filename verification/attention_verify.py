...
if __name__ == '__main__':
    ...
    context_network = nn.Linear(embed_dim * (1 + emergency_count), emergency_count).to(device)
    ...
    labels = torch.argmin(torch.norm(mock_emergencies - agent_pos, dim=-1), dim=-1).to(torch.long).to(device)
    epochs = 100
    ...
    optimizer = torch.optim.Adam(all_parameters, lr=1e-2)
    inputs = torch.cat((mock_emergencies, agent_pos), dim=1)
