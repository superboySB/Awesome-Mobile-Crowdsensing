...
def train():
    return tune.Tuner(
        ...
        run_config=air.RunConfig(stop=stop, verbose=1,
                                 storage_path=os.path.join("/workspace", "saved_data", "ray_results")),
    ).fit()
...
if __name__ == "__main__":
    ...
    ray.init(num_cpus=args.num_cpus or None,
             ...
             _temp_dir=os.path.join("/workspace", "saved_data", "tmp", "ray"))
    ...
    config = (
        PPOConfig()
        .environment(RLlibCrowdSim, env_config=run_config['env'])
        .resources()
