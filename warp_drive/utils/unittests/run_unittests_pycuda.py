...
if __name__ == "__main__":
    ...
    cmds = [
        f"pytest {get_project_root()}/tests/warp_drive --timeout=300",
        f"pytest {get_project_root()}/tests/example_envs --timeout=300",
    ]
    for cmd in cmds:
    ...
    cmds = [
        f"pytest {get_project_root()}/tests/multiblocks_per_env/warp_drive --timeout=300",
        f"pytest {get_project_root()}/tests/multiblocks_per_env/example_envs --timeout=300",
    ]
    for cmd in cmds:
        print(f"Running Unit tests: {cmd} ")
        with subprocess.Popen(
